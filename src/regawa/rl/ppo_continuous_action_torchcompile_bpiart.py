...
class BipartiteData(Data):
    def __inc__(self, key, value, *args, **kwargs):
        if key == "edge_index":
            return torch.tensor(
                [[self.ground_class.size(0)], [self.object_class.size(0)]]
            )
...
def dict_to_data(data: dict[str, tuple[Any]], num_envs: int) -> list[Data]:
    return [
        BipartiteData(
            ground_class=torch.as_tensor(data["predicate_class"][n], dtype=torch.int64),
            ground_value=torch.as_tensor(
                data["predicate_value"][n], dtype=torch.float32
            ),
            object_class=torch.as_tensor(data["object"][n], dtype=torch.int64),
            ...
            num_nodes=data["predicate_class"][n].shape[0],
            dtype=torch.int64,
        )
...
class KGAgent(nn.Module):
    def __init__(self, envs, device: str | None = None):
        super().__init__()
        ...
        self.gnn_agent = KGGNNAgent(n_types, n_relations, n_actions)
...
class BPAgent(nn.Module):
    def __init__(self, envs, device: str | None = None):
        ...
        n_types = envs.single_observation_space.spaces["object"].high[0]
        n_relations = envs.single_observation_space.spaces["predicate_class"].high[0]

        self.gnn_agent = GraphActorCritic(n_types, n_relations)
    ...
    def get_value(
        ...
        _, _, _, value = self.gnn_agent(x_p, x_c, x_o, edge_index, edge_attr, batch_idx)
        return value
    ...
    def get_action_and_value(
        ...
        a, logprob, entropy, value = self.gnn_agent.forward(
            ground_value, ground_class, object_class, edge_index, edge_attr, batch_idx
        )
        return (
            a,
            logprob,
...
def gae(
    next_obs: dict[str, tuple[Any]],
    next_done: Tensor,
    container: tensordict.TensorDict,
    gamma: float,
    gae_lambda: float,
    num_steps: int,
    get_value: Callable[[Tensor, Tensor, Tensor, Tensor], Tensor],
    ...
    for t in range(num_steps - 1, -1, -1):
        ...
        delta = rewards[t] + gamma * nextvalues * nextnonterminal - cur_val
        advantages.append(delta + gamma * gae_lambda * nextnonterminal * lastgaelam)
...
def rollout(
    ...
    device: str,
    step_func: Callable[
        [Tensor], tuple[dict[str, Any], Tensor, Tensor, dict[str, Any]]
    ],
):
    ...
    for _ in range(num_steps):
        ...
        data.extend(obs)
        next_obs = dict_to_data(next_obs, num_envs)
...
def update(
    ...
    _, newlogprob, entropy, newvalue = agent.get_action_and_value(
        x_p, x_c, x_o, edge_index, edge_attr, batch_idx, actions
    )
...
def main():
    ...
    ####### Agent #######
    agent = BPAgent(envs, device=device)
    # Make a version of agent with detached params
    agent_inference = BPAgent(envs, device=device)
    ...
    # Compile policy
    if args.compile:
        policy_f = torch.compile(policy)
        gae_f = torch.compile(gae, fullgraph=True)
        update_f = torch.compile(update)
    else:
        gae_f = gae
        update_f = update
        policy_f = policy
    ...
    if args.cudagraphs:
        policy_f = CudaGraphModule(policy)
        gae_f = CudaGraphModule(gae)
        update_f = CudaGraphModule(update)
    else:
        policy_f = policy
        gae_f = gae
        update_f = update
        ...
        for epoch in range(args.update_epochs):
            ...
            for b in b_inds:
                ...
                out = update_f(
                    optimizer,
                    agent,
                    minibatch.ground_value,
                    minibatch.ground_class,
                    minibatch.object_class,
